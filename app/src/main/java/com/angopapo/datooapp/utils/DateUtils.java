...
public class DateUtils {
    ...
    public static String getTimeAgo(long time) {
        ...
        if (diff < MINUTE_MILLIS) {
            return Application.getInstance().getString(R.string.time_now);
        } else if (diff < 2 * MINUTE_MILLIS) {
            return Application.getInstance().getString(R.string.time_one_minute_ago);
        } else if (diff < 50 * MINUTE_MILLIS) {
            return diff / MINUTE_MILLIS + " " + Application.getInstance().getString(R.string.time_minutes_ago);
        } else if (diff < 90 * MINUTE_MILLIS) {
            return Application.getInstance().getString(R.string.time_one_hour_ago);
        } else if (diff < 24 * HOUR_MILLIS) {
            return diff / HOUR_MILLIS + " " + Application.getInstance().getString(R.string.time_hours_ago);
        } else if (diff < 48 * HOUR_MILLIS) {
            return Application.getInstance().getString(R.string.time_yesterday);
        } else {
            if ((diff / DAY_MILLIS) > 365) {
                return ((int) ((diff / DAY_MILLIS) / 365)) + " " + Application.getInstance().getString(R.string.time_years_ago);
            } else if ((diff / DAY_MILLIS) > 31) {
                return ((int) (diff / DAY_MILLIS) / 31) + " " + Application.getInstance().getString(R.string.time_months_ago);
            } else {
                return diff / DAY_MILLIS + " " + Application.getInstance().getString(R.string.time_days_ago);
            }
        }
    }
